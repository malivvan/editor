...
func (c *CellView) Draw(buf *Buffer, colorscheme Colorscheme, top, height, left, width int) {
	...
	if settingAs(buf.Settings, "matchbrace", false) {
		for _, bp := range bracePairs {
			curX := buf.Cursor.X
			curLoc := buf.Cursor.Loc
			if settingAs(buf.Settings, "matchbraceleft", false) {
				if curX > 0 {
					curX--
	...
	tabsize := int(settingAs(buf.Settings, "tabsize", float64(4)))
	softwrap := settingAs(buf.Settings, "softwrap", false)
	indentrunes := []rune(settingAs(buf.Settings, "indentchar", " "))
	...
	start := buf.lastEditLine
	if start >= len(buf.lines) {
		start = len(buf.lines) - 1
		if start < 0 {
			start = 0
		}
	}
	if settingAs(buf.Settings, "syntax", true) && buf.syntaxDef != nil {
		if start > 0 && start-1 < len(buf.lines) && buf.lines[start-1].rehighlight {
		...
		for viewCol < lineLength {
			...
			if char == '\t' {
				...
				if viewCol >= 0 {
					...
					ch := settingAs(buf.Settings, "indentchar", " ")
	...
	// Only invalidate matches for lines that were actually edited.
	for lineN := range buf.dirtyLines {
		if lineN >= 0 && lineN < len(buf.lines) {
			buf.SetMatch(lineN, nil)
		}
	}
	// Clear the dirty set for the next frame.
	for k := range buf.dirtyLines {
		delete(buf.dirtyLines, k)
	}
}
