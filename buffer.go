...
type Buffer struct {
	...
	*LineArray
	...
	// Whether the buffer has been modified since it was opened
	IsModified bool
	...
	// lastEditLine is the line number of the most recent edit,
	// used as the rehighlight start point instead of the cursor position.
	lastEditLine int

	// dirtyLines tracks which lines were modified since the last Draw.
	// CellView.Draw clears matches only for these lines instead of the
...
func NewBuffer(reader io.Reader, size int64, path string, cursorPosition []string) *Buffer {
	...
	b.dirtyLines = make(map[int]struct{})
	...
	if !settingAs(b.Settings, "fastdirty", true) {
...
func (b *Buffer) updateRules(runtimeFiles *RuntimeFiles) {
	...
	for _, f := range runtimeFiles.ListRuntimeFiles(RTSyntax) {
		...
		if err == nil {
			...
			ft := settingAs(b.Settings, "filetype", "Unknown")
	...
	if b.highlighter == nil || rehighlight {
		if b.syntaxDef != nil {
			...
			if settingAs(b.Settings, "syntax", true) {
...
func (b *Buffer) FileType() string {
	return settingAs(b.Settings, "filetype", "Unknown")
}

// IndentString returns a string representing one level of indentation
func (b *Buffer) IndentString() string {
	if settingAs(b.Settings, "tabstospaces", false) {
		return Spaces(int(settingAs(b.Settings, "tabsize", float64(4))))
	}
	return "\t"
}
...
// into one cursor. Uses sort + single-pass scan (O(k log k)) instead
// of the previous O(k²) nested loop.
func (b *Buffer) MergeCursors() {
	...
	// Sort cursors by (Y, X) position.
	sort.Slice(b.cursors, func(i, j int) bool {
		ci, cj := b.cursors[i], b.cursors[j]
		if ci.Y != cj.Y {
			return ci.Y < cj.Y
		}
		return ci.X < cj.X
	...
	// Single-pass: keep only unique positions. When duplicates are found,
	// keep the first (lowest original index after sort) and discard others.
	merged := make([]*Cursor, 0, len(b.cursors))
	merged = append(merged, b.cursors[0])
...
// MarkDirty marks a line as needing match invalidation on the next Draw.
func (b *Buffer) MarkDirty(lineN int) {
	if b.dirtyLines != nil {
		b.dirtyLines[lineN] = struct{}{}
	}
}

func (b *Buffer) insert(pos Loc, value []byte) {
	b.IsModified = true
	b.lastEditLine = pos.Y
	b.LineArray.insert(pos, value)
	// Mark the edited line and any new lines created by the insert.
	for i := pos.Y; i < len(b.lines) && i <= pos.Y+countNewlines(value); i++ {
		b.MarkDirty(i)
	}
	b.update()
}
