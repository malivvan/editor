// Package highlight is the syntax highlighter behind the editor.
//
// It reads micro-compatible syntax definitions — YAML files with a file-type
// header and a list of rules per syntax group — and turns them into line
// matches: for every line, a map from the byte position where the colour
// changes to the Group that applies from there on. The editor consumes those
// matches in its CellView, which resolves each group to a tcell.Style through
// the active colorscheme.
//
// Parsing and highlighting are separate steps, which is what makes incremental
// rehighlighting possible:
//
//	file, err := highlight.ParseFile(data)     // the rules of one grammar
//	header, err := highlight.ParseHeader(data) // how to detect the file type
//	def, err := highlight.ParseDef(file, header)
//	highlight.ResolveIncludes(def, files)      // pull in "include" rules
//
//	h := highlight.NewHighlighter(def)
//	h.HighlightStates(buf)                     // end-of-line state per line
//	h.HighlightMatches(buf, 0, buf.LinesNum()) // groups per line
//
// The buffer passed in must implement LineStates: a way to store, and read
// back, the highlighter state at the end of every line. States are what keep a
// construct that spans lines — a block comment, a multi-line string — correct
// when only part of the buffer is re-highlighted, and ReHighlightStates and
// ReHighlightLine are the incremental entry points the editor uses for that.
package highlight
