...
import (
	...
	"github.com/malivvan/editor/util"
...
func (eh *EventHandler) Insert(start Loc, textStr string) {
	...
	for _, c := range eh.buf.cursors {
		move := func(loc Loc) Loc {
			if start.Y != end.Y {
				if loc.Y == start.Y && loc.GreaterEqual(start) {
					// Cursor is on the same line as the insert, after
					// the insert point. Shift to the new end line and
					// adjust X by the difference between the end column
					// and the start column.
					loc.X = (loc.X - start.X) + end.X
					loc.Y += end.Y - start.Y
...
func (eh *EventHandler) Remove(start, end Loc) {
	...
	for _, c := range eh.buf.cursors {
		move := func(loc Loc) Loc {
			if start.Y != end.Y {
				if loc.Y == end.Y && loc.GreaterEqual(end) {
					// Cursor is on the end line of the removal, at or
					// after the end column. Shift to the start line and
					// adjust X by the difference between the start and
					// end columns.
					loc.X = (loc.X - end.X) + start.X
					loc.Y -= end.Y - start.Y
				} else if loc.GreaterThan(end) {
					loc.Y -= end.Y - start.Y
				}
...
func (eh *EventHandler) Execute(t *TextEvent) {
	...
	// Record the edit point for rehighlighting.
	if len(t.Deltas) > 0 {
		eh.buf.lastEditLine = t.Deltas[0].Start.Y
	}

	ExecuteTextEvent(t, eh.buf)
	eh.buf.NumLines = len(eh.buf.lines)

	// Trim the undo stack if it exceeds the configured limit.
	maxUndo := int(settingAs(eh.buf.Settings, "maxundohistory", float64(10000)))
	if maxUndo > 0 {
		eh.UndoStack.Trim(maxUndo)
	}
}
